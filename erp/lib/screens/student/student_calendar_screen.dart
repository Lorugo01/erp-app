...
class _StudentCalendarScreenState extends State<StudentCalendarScreen> {
      ...
      if (studentId != null) {
        ...
        if (response.statusCode == 200) {
          ...
          final currentEnrollments =
              data
                  .where((enrollment) => enrollment['current'] == true)
                  .toList();

          setState(() {
            _classes = List<Map<String, dynamic>>.from(currentEnrollments);
            if (_classes.isNotEmpty && _selectedClass == null) {
              _selectedClass = _classes.first;
              _selectedClassId = _selectedClass!['class']['id'];
              _loadLessons();
            }
          });
        } else {
        ...
        setState(() {
          _errorClasses = 'ID do aluno não encontrado';
        });
      }
    } catch (e) {
  ...
  void _selectClass(Map<String, dynamic> classData) {
    setState(() {
      ...
      _selectedClassId = classData['class']['id'];
    });
    _loadLessons();
  }
